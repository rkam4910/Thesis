...
# Radar noise floor: estimate from bins just beyond where clutter disappears
# (bins with no returns = below noise floor). Use the last valid clutter value
# as a proxy for the detection threshold at that depth.
last_valid_veg = np.nanmin(radar_veg_dB[veg_mask_plot])   # lowest recorded clutter
NOISE_FLOOR_DB = last_valid_veg - 2                        # ~2 dB below last detection

# Noise floor as a horizontal dashed line across the full depth range
ax5.axhline(NOISE_FLOOR_DB, color='gray', linestyle='--', lw=1.2, alpha=0.8,
            label=f'Radar detection threshold (~{NOISE_FLOOR_DB:.0f} dB)')
ax5.text(0.35, NOISE_FLOOR_DB + 1.0, 'Radar detection threshold\n'
         '(vegetation clutter drops below this beyond ~3.3 m)',
         fontsize=7.5, color='gray', style='italic')

# Radar vegetation zone — line only through the reed body
